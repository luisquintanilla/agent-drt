...
public partial class DurableFunctionInvokingChatClient : DelegatingChatClient
{
    ...
    private static readonly AsyncLocal<ToolCallContext?> s_currentContext = new();
        ...
        IEnumerable<ChatMessage> messages, ChatOptions? options = null, CancellationToken cancellationToken = default)
    {
        ...
        using Activity? activity = this._activitySource?.StartActivity(nameof(DurableFunctionInvokingChatClient));

        // Load persisted messages if a persistence key is provided
        string? persistenceKey = options.GetPersistenceKey();
        string? currentETag = null;
        if (persistenceKey is not null && this._messagePersistence is not null)
        {
            (IList<ChatMessage> persistedMessages, string? eTag) = await this._messagePersistence.FetchMessagesAsync(persistenceKey, cancellationToken).ConfigureAwait(false);
            currentETag = eTag;
        ...
        for (int iteration = 0; ; iteration++)
        {
            ...
            {
                // Persist messages before returning
                if (persistenceKey is not null && this._messagePersistence is not null)
                {
                    List<ChatMessage> allMessages = [.. messages, .. response.Messages];
                    currentETag = await this._messagePersistence.AppendMessagesAsync(persistenceKey, allMessages, currentETag, cancellationToken).ConfigureAwait(false);
                }
            ...
            {
                // Persist messages before breaking
                if (persistenceKey is not null && this._messagePersistence is not null && responseMessages is not null)
                {
                    List<ChatMessage> allMessages = [.. originalMessages, .. responseMessages];
                    currentETag = await this._messagePersistence.AppendMessagesAsync(persistenceKey, allMessages, currentETag, cancellationToken).ConfigureAwait(false);
                }
            ...
            // Persist messages after function invocation
            if (persistenceKey is not null && this._messagePersistence is not null)
            {
                List<ChatMessage> allMessages = [.. originalMessages, .. responseMessages];
                currentETag = await this._messagePersistence.AppendMessagesAsync(persistenceKey, allMessages, currentETag, cancellationToken).ConfigureAwait(false);
            ...
            if (UpdateOptionsForMode(modeAndMessages.Mode, ref options!, response.ConversationId))
            {
                // Terminate
                break;
            }
        ...
        IEnumerable<ChatMessage> messages, ChatOptions? options = null, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        for (int iteration = 0; ; iteration++)
        {
            ...
            await foreach (var update in base.GetStreamingResponseAsync(messages, options, cancellationToken).ConfigureAwait(false))
            {
                ...
                // Persist messages before yielding (done after reconstituting full response below)
                yield return update;
            ...
            // Persist messages after receiving full response
            if (persistenceKey is not null && this._messagePersistence is not null)
            {
                List<ChatMessage> allMessages = [.. originalMessages, .. responseMessages];
                currentETag = await this._messagePersistence.AppendMessagesAsync(persistenceKey, allMessages, currentETag, cancellationToken).ConfigureAwait(false);
    ...
    private static bool UpdateOptionsForMode(ContinueMode mode, ref ChatOptions options, string? chatThreadId)
    {
        switch (mode)
        {
            ...
            default:
                ...
                if (options.ConversationId != chatThreadId)
                {
                    options = options.Clone();
                    options.ConversationId = chatThreadId;
                }
    ...
    private void ThrowIfNoFunctionResultsAdded(IList<ChatMessage>? messages)
    {
        ...
        {
            throw new InvalidOperationException($"{this.GetType().Name}.{nameof(CreateResponseMessages)} returned null or an empty collection of messages.");
        }
    }
    ...
    protected virtual async Task<object?> InvokeFunctionAsync(ToolCallContext context, CancellationToken cancellationToken)
    {
        ...
        if (this._logger.IsEnabled(LogLevel.Debug))
        {
            ...
            {
                this.LogInvokingSensitive(context.Function.Name, LoggingHelpers.AsJson(context.CallContent.Arguments, context.Function.JsonSerializerOptions));
            }
            else
            {
